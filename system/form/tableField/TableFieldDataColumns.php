...
class TableFieldDataColumns implements TableField_ColumnProvider
{
    ...
    public function setFieldCasting($fieldCasting)
    {
        if (is_array($fieldCasting)) {
            $this->fieldCasting = ArrayLib::map_key("strtolower", $fieldCasting);
    ...
    public function getColumnContent($tableField, $record, $columnName)
    {
        ...
        $value = $this->castValue($tableField, $record, $columnName, $value);
     ...
     * @return string
     */
    protected function castValue($tableField, $record, $fieldName, $value)
    {
        if(PROFILE) Profiler::mark("tableFieldDataColumns::castValue");

        $casting = gObject::method_exists($record, "casting") ? array_merge(
            $record->casting(),
            $this->fieldCasting
        ) : $this->fieldCasting;

        // If a fieldCasting is specified, we assume the result is safe
        if (array_key_exists(strtolower($fieldName), $casting)) {
            $value = $tableField->getCastedValue($value, $casting[strtolower($fieldName)]);
        ...
        if(PROFILE) Profiler::unmark("tableFieldDataColumns::castValue");
