...
class Resources extends gObject
{
    ...
    public static function add($content, $type = null, $combine_name = null, $lessVars = null)
    {
        ...
        switch ($type) {
            ...
            case "js":
            case "javascript":
                if ($combine_name != "" && !RegexpUtil::checkFileExt(
                        $content,
                        "php"
                    ) && $path === true /* file exists */) {
    ...
    /**
     * gets the resources
     * @param bool $shouldCombineJS defines if js should be combined to less files
     * @param bool $asArray defines if it should return array or string of HTML
     * @param bool $css
     * @param bool $js
     * @return string|array
     */
    public static function get($shouldCombineJS, $asArray, $css = true, $js = true)
    {
        ...
        // generate files
        $files = self::generateFiles($shouldCombineJS, $css, $js);
        $js = $files[1];
        $css = $files[0];

        if ($js && self::$registeredResources["js"]) {
            self::$resources_data[] = "goma.ui.registerResources('js', ".json_encode(
                    array_values(self::$registeredResources["js"])
                ).");";
        }

        if ($css && self::$registeredResources["css"]) {
            self::$resources_data[] = "goma.ui.registerResources('css', ".json_encode(
                    array_values(self::$registeredResources["css"])
                ).");";
        }
        ...
        if ($asArray) {
            ...
            if ($js) {
                ...
                $html .= "			<script type=\"text/javascript\" src=\"".ROOT_PATH.$langFile."?".filemtime(
                        ROOT.$langFile
                    )."\"></script>\n";
    ...
    public static function generateCSSFile($combine_css, $name = "", &$css_files)
    {
        ...
        $file = self::getFileName(
            CACHE_DIRECTORY."css_".$name."_".md5($lessStr)."_".md5(
                implode("_", $combine_css["files"])
            )."_".$combine_css["mtime"]."_".preg_replace('/[^a-zA-Z0-9_]/', '_', self::VERSION).$debugStr.".css"
        );
...
*/\n\n";
            foreach ($combine_css["files"] as $cssfile) {
                ...
                if (self::file_exists($cachefile) && filemtime($cachefile) > filemtime(ROOT.$cssfile)) {
                    ...
                    $data .= trim(
                            self::parseCSS(
                                file_get_contents($less)."\n\n".file_get_contents(ROOT.$cssfile),
                                $cssfile,
                                ROOT_PATH
                            )
                        )."\n\n";
     ...
     * @param bool $shouldCombineJS
     * @param bool $css
     * @param bool $js
     * @return array
     */
    public static function generateFiles($shouldCombineJS, $css = true, $js = true)
    {
        ...
        if ($js) {
            if (!$shouldCombineJS) {
                ...
                if (count(self::$raw_js) > 0) {
                    $file = self::getFileName(
                        ROOT.CACHE_DIRECTORY."raw_".md5(implode("", self::$raw_js))."_".preg_replace(
                            '/[^a-zA-Z0-9_]/',
                            '_',
                            self::VERSION
                        ).$debugStr.".js"
    ...
    {
        if (PROFILE) {
            Profiler::mark("Resources::makeCombinedJS");
        }
        ...
        $file = self::getFileName(
            CACHE_DIRECTORY."js_combined_".$data["name"]."_".$hash."_".$data["mtime"]."_".preg_replace(
                '/[^0-9a-zA-Z_]/',
                '_',
                self::VERSION
            ).$debugStr.".js"
...
*/\n\n";
    ...
    public static function file_exists($file)
    {
        ...
        if (defined("CLASS_INFO_LOADED")) {
            if (!strpos($file, "../") && preg_match('/\.(js|css|html)$/i', $file) && (substr(
                        $file,
                        0,
                        strlen(SYSTEM_TPL_PATH)
                    ) == SYSTEM_TPL_PATH || substr($file, 0, strlen(APPLICATION_TPL_PATH)) == APPLICATION_TPL_PATH)) {
    ...
    {
        if (PROFILE) {
            Profiler::mark("Resources::GenerateClassInfo");
        }
        ...
        if (PROFILE) {
            Profiler::unmark("Resources::GenerateClassInfo");
        }
    }
