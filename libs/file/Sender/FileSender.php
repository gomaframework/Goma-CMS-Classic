...
class ResumeDownload {
    ...
    function __construct($file, $delay = 0, $name = null) {
        if (! is_file($file)) {
        ...
        $this->delay = $delay;
        $this->name = isset($name) ? $name : basename($file);
    }
    ...
    private function readFile() {
        while ( ! feof($this->file) && !$this->userAborted() ) {
    ...
    private function readBuffer($bytes, $size = 1024) {
        $bytesLeft = $bytes;
        while ( $bytesLeft > 0 && ! feof($this->file) && !$this->userAborted() ) {
...
if(file_exists("../../../temp/download." . basename($_GET["downloadID"]) . ".goma")) {
	...
	if(isset($data["file"]) && file_exists($data["file"])) {
        ...
        $download = new ResumeDownload($file, 0, $filename);
        $download->process();
    }
} else {
	echo "File not found";
